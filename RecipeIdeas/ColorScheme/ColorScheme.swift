//
//  ColorScheme.swift
//  RecipeIdeas
//
//  Created by Erik HARTLEY on 3/2/18.
//  Copyright © 2018 Erik HARTLEY. All rights reserved.
//

import UIKit

class ColorScheme {
    let bunting = UIColor(red:0.18, green:0.19, blue:0.26, alpha:1.0)
    let jungleMist = UIColor(red:0.75, green:0.75, blue:0.75, alpha:1.0)
    let white = UIColor(red:1.00, green:1.00, blue:1.00, alpha:1.0)
    let crusta = UIColor(red:0.94, green:0.51, blue:0.33, alpha:1.0)
    let slateGrey = UIColor(red:0.31, green:0.36, blue:0.46, alpha:1.0)
}
